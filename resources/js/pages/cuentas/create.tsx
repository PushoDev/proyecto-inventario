...
export default function CreateCuenta() {
    ...
    const { data, setData, post, processing, errors } = useForm({
        nombre_cuenta: '',
        saldo_cuenta: null as number | null, // Cambio aquí: saldo_cuenta puede ser null o number
        tipo_cuenta: '',
        notas_cuenta: '',
    });
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Crear Cuenta Monetaria" />
            <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
                {/* Encabezado */}
                <div className="*:data-[slot=card]:from-primary/5 *:data-[slot=card]:to-card dark:*:data-[slot=card]:bg-card grid grid-cols-1 gap-4 px-4 *:data-[slot=card]:bg-gradient-to-t *:data-[slot=card]:shadow-xs lg:px-6 @xl/main:grid-cols-2 @5xl/main:grid-cols-4">
                    <header>
                        <div className="relative col-span-4 space-y-1 overflow-hidden rounded-2xl border border-dashed border-gray-700 bg-gray-800 p-4">
                            <HeadingSmall title="Crear Nueva Cuenta" description="Agrega una nueva cuenta fiscal, monetaria o temporal al sistema" />
                            <HandCoins
                                size={70}
                                color="green"
                    ...
                    <div className="rounded-lg border border-gray-700 bg-gray-800 p-6 shadow-md">
                        ...
                        <div className="mb-4">
                            ...
                            <Input
                                ...
                                type="number"
                                step="0.01"
                                value={data.saldo_cuenta ?? ''} // Usamos nullish coalescing para manejar null
                                onChange={(e) => {
                                    const value = e.target.value.trim();
                                    setData('saldo_cuenta', value === '' ? null : parseFloat(value)); // Manejamos valores vacíos como null
                                }}
